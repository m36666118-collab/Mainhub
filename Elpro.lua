...
AutoFarm:AddSwitch("Fuerza OP (Usalo si tu ping mínimo es de 250 ms)", function(state)
...
-- Switch en la librería
AutoFarm:AddSwitch("Fuerza Op (Usalo si tu ping mínimo es inferior a 150 ms)", function(state)
    getgenv()._AutoRepFarmEnabled = state
    warn("[Auto Rep Farm] Estado cambiado a:", state and "ON" or "OFF")
end)
...
AutoFarm:AddSwitch("Auto Comer Huevo de Proteína cada 30 Minutos", function(state)
    autoEatEnabled = state
end)


    AutoFarm:AddSwitch("Auto Girar Ruleta", function(state)
...
-- ✅ Optimización visual
AutoFarm:AddSwitch("Ocultar todos los Iconos", function(bool)
...
local fastStrengthFolder = AutoFarm:AddFolder("Funcion de Rebirth Rapido")
...
local footerRebirthsGainedLabel = fastStrengthFolder:AddLabel("Rebirths Ganados: 0")
...
fastStrengthFolder:AddSwitch("Rebirth Rapidos", function(state)
...
local rebirthFolder = AutoFarm:AddFolder("Rebirth Sin Paquetes")
...
rebirthFolder:AddTextBox("Número objetivo de renacimientos", function(value)
...
local targetRebirthToggle = rebirthFolder:AddSwitch("Iniciar", function(enabled)
...
rebirthFolder:AddSwitch("Auto Teletransportarse al Muscle King", function(enabled)
    if enabled then
...
rebirthFolder:AddSwitch("Auto Colocar Tamaño 2", function(enabled)
    if enabled then
        _G.AutoSize2 = true
...
local autoEquipToolsFolder = AutoFarm:AddFolder("Ejercicios Automaticos")
...
local PlayerDrop = estadisticas:AddDropdown("Seleccionar Jugador", function(Value)
        ...
        if SelectPlayerName ~= "" then
            ...
            if player then
                ...
                if player:FindFirstChild("customSize") then
                    Update10.Text = "Tamaño Personalizado: " .. FormatDisplay(player.customSize.Value)
                end
                ...
                if player:FindFirstChild("currentMap") then
                    ...
                    Update9.Text = "Mapa actual: Aucune donnÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©e"
                end
            end
        end
    end
...
estadisticas:AddLabel("Estadísticas Avanzadas:").TextSize = 24
...
local function updateAdvancedStats(targetPlayer)
    ...
    playerDamageLabel.Text = "Mi Daño: " .. FormatDisplay(playerDamage)
...
local titleLabel = Killer:AddLabel("Seleccione PET de daño o durabilidad")
titleLabel.TextSize = 18
titleLabel.Font = Enum.Font.Merriweather 
titleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)

local dropdown = Killer:AddDropdown("Seleccionar Pet", function(text)
...
Killer:AddSwitch("Auto Karma Bueno", function(bool)
    autoGoodKarma = bool
    task.spawn(function()
...
Killer:AddSwitch("Auto Karma Malvado", function(bool)
    autoBadKarma = bool
    task.spawn(function()
...
Killer:AddSwitch("Auto Lista Blanca de Amigos", function(state)
...
Killer:AddTextBox("Lista Blanca", function(text)
...
Killer:AddTextBox("Lista no Blanca", function(text)
    local target = Players:FindFirstChild(text)
    if target then
...
local targetDropdown = Killer:AddDropdown("Seleccionar Objetivo", function(displayName)
    for _, player in ipairs(Players:GetPlayers()) do
...
Killer:AddButton("Remover Objetivo Seleccionado", function()
...
Killer:AddSwitch("Iniciar Kill al Objetivo", function(state)
    killTarget = state
...
local spyTargetDropdown = Killer:AddDropdown("Seleccionar para Espectear objetivo", function(displayName)
    for _, player in ipairs(Players:GetPlayers()) do
        if player.DisplayName == displayName then
...
local button = Killer:AddButton("Remover Animacion de Puños", function()
...
Killer:AddButton("Reactivar Animacion de Puño", function()
    RecoveryPunch()
end)

Killer:AddSwitch("Auto Equipar Puños", function(state)
...
Killer:AddSwitch("Golpe automático [sin animación]", function(state)
...
Killer:AddSwitch("Auto Puños", function(state)
...
Killer:AddSwitch("Puños Rapidos", function(state)
...
Killer:AddSwitch("Modo Dios", function(State)
    godModeToggle = State
    if State then
        task.spawn(function()
...
local followDropdown = Killer:AddDropdown("teletransportarse al Jugador", function(selectedDisplayName)
...
Killer:AddButton("Activar Golpe Muerto", function()
...
-- Dropdown
local timeDropdown = Killer:AddDropdown("Cambiar de Hora", function(selection)
...
local FolderDamage = Calculadora:AddFolder("Calculadora de Daño con Paquete")

FolderDamage:AddTextBox("Base de Fuerza (ej: 1.27Qa, T, B)", function(text)
...
FolderDamage:AddButton("Calcular Daño", function()
...
local FolderDurabilidad = Calculadora:AddFolder("Calculadora de Durabilidad con Paquete")

FolderDurabilidad:AddTextBox("Base de Durabilidad (ej: 1.27Qa, T, B)", function(text)
    local unidades = { ["T"] = 1e12, ["Q"] = 1e15, ["B"] = 1e9 }
    text = text:upper()
    for u, m in pairs(unidades) do
        if text:find(u) then
...
FolderDurabilidad:AddButton("Calcular Durabilidad", function()
...
local stopwatchLabel = features:AddLabel("Tiempo de Repetición Rápido: 0d 0h 0m 0s")
stopwatchLabel.TextSize = 20

local projectedStrengthLabel = features:AddLabel("Tasa de Fuerza: 0 /Hora | 0 /Dia | 0 /Semana | 0 /Mes")
projectedStrengthLabel.TextSize = 20

local projectedDurabilityLabel = features:AddLabel("Taza de Durabilidad: 0 /Hora | 0 /Dia | 0 /Semana | 0 /Mes")
projectedDurabilityLabel.TextSize = 20

features:AddLabel("").TextSize = 10

local statsLabel = features:AddLabel("Estadisticas:")
...
local pets = window:AddTab(       "TIENDAS")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
pets:AddLabel("Comprar ultimates")
...
-- Ultimate dropdown
local ultimateDropdown = pets:AddDropdown("Seleccionar ultimate", function(text)
...
pets:AddSwitch("Auto Comprar Ultimates", function(bool)
...
farmTab:AddLabel("Aqui Puedes Farmear rocas depende tu REBIRTH")
...
local tinyIslandRockSwitch = farmTab:AddSwitch("Isla Pequeña - 0-1k Fuerza", function(bool)
...
Gift:AddLabel("Regalar Huevo Proteina:").TextSize = 22

local proteinEggLabel = Gift:AddLabel("Huevos de Proteina: 0")
proteinEggLabel.TextSize = 20

local selectedEggPlayer = nil
local eggCount = 0

local eggDropdown = Gift:AddDropdown("Elije Jugador para Regalar Huevos", function(selectedDisplayName)
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.DisplayName == selectedDisplayName then
...
Gift:AddTextBox("Cantidad de Huevos", function(text)
    eggCount = tonumber(text) or 0
end)

Gift:AddButton("Regalar Huevos", function()
...
Gift:AddLabel("Regala Batidos Tropicales:").TextSize = 22

local tropicalShakeLabel = Gift:AddLabel("Batidos Tropical: 0")
tropicalShakeLabel.TextSize = 18

local selectedShakePlayer = nil
local shakeCount = 0

local shakeDropdown = Gift:AddDropdown("Elije Jugador para Regalar Batidos", function(selectedDisplayName)
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr.DisplayName == selectedDisplayName then
...
Gift:AddTextBox("Regalo de Batidos Tropicales", function(text)
    shakeCount = tonumber(text) or 0
end)

Gift:AddButton("Regalar Batidos Tropical", function()
...
Credits:AddLabel("OMEGADEZZ Paid / Script OP")
Credits:AddLabel("Script Creado por:")
Credits:AddLabel("HonnyDezz / Colaborador Arlexuin")
Credits:AddLabel("Discord : https://discord.gg/TQchfR6wse")
